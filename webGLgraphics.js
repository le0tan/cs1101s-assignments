...
function initShader(programName) {
  var vertexShader;
  if (
    !(vertexShader = getShader(gl, programName + '-vertex-shader', 'vertex'))
  ) {
    return null;
  }
  var fragmentShader;
...
module.exports = {clear_viewport: function () {
  if (!gl) {
    throw new Error(
...
function initRune3d() {
  ...
  u_cameraMatrix = gl.getUniformLocation(
    anaglyphShaderProgram,
    'u_cameraMatrix'
  );
  ...
  if (
    !(
      (leftFramebuffer = initFramebufferObject()) &&
...
function drawCurve(drawMode, curvePosArray) {
  ...
  for (var i = 0; i <= curvePosArray.length / magicNum / itemSize; i++) {
    ...
    var subArray = curvePosArray.slice(
      i * magicNum * itemSize,
      (i + 1) * magicNum * itemSize
    );
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
