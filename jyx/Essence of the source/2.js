...
//function greater overloads >
function greater(x, y) {
    if ((is_function_object(x) || is_builtin_function(x)) && is_list(y)) {
        return map(e => apply(x, list(e)), y); //if x is a function and y is a list apply map function
    } else {
        return x > y; // otherwise the normal >
    }
}
...
const builtin_functions = list(
    ...
    pair("<", (x, y) => x < y),
    pair("<=", (x, y) => x <= y),
    pair(">", greater), // overload > in function greater
    pair(">=", (x, y) => x >= y),
    pair("!", x => !x)
);
