...
function evaluate_constant_declaration(stmt, env) {
    let val = constant_declaration_value(stmt);
    if(is_self_evaluating(val)) {
...
function evaluate_variable_declaration(stmt, env) {
    let val = variable_declaration_value(stmt);
    if(is_self_evaluating(val)) {
        val = evaluate(val,env);
    } else {
        val = wrap(val, env);
...
function apply_builtin_function(fun, argument_list) {
    const arg = map(x => is_wrap(x)? evaluate_wrap(x):x, argument_list);
    return apply_in_underlying_javascript(
        builtin_implementation(fun),
        map(x => is_wrap(x) ? evaluate_wrap(x) : x,arg));
}
...
function evaluate_return_statement(stmt, env) {
    let val = return_statement_expression(stmt);
    if(is_self_evaluating(val)) {
        val = evaluate(val);
    } else {
        val = wrap(val, env);
    }
    return make_return_value(val);
}
...
function evaluate_assignment(stmt, env) {
    let val = assignment_right_hand_side(stmt);
    if(is_self_evaluating(val)) {
        val = evaluate(val,env);
    } else {
        val = wrap(val, env);
    }
    assign_name_to_value(assignment_name(stmt), val, env);
    return is_wrap(val) ? evaluate_wrap(val) : val;
}
...
function lookup_name_value(name, env) {
    function env_loop(env) {
        if (is_empty_environment(env)) {
            ...
            const val = first_frame(env)[name].value;
            if(is_wrap(val)) {
...
function evaluate(stmt, env) {
    ...
    } else if(is_wrap(stmt)){
        return evaluate_wrap(stmt);
    }else {
...
function is_wrap(stmt) {
    return is_tagged_object(stmt, "wrap");
}

function wrap(stmt, env) {
    return {tag:"wrap", value:stmt, environment:env};
}

function evaluate_wrap(stmt) {
    return evaluate(stmt.value, stmt.environment);
}
