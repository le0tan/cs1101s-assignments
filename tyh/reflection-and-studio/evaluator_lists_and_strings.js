...
out simple arithmetic calculations, boolean calculations and 
list manipulations and supports if statements (but no declarations, 
or functions)
...
function evaluate_boolean_operation(stmt) {
    if (operator(stmt) === "&&") {  
        if (is_true(evaluate(first_operand(operands(stmt))))) {
            return evaluate(
                      first_operand(rest_operands(operands(stmt))));
        } else {
            return false;
        } 
    } else {
...
// the parse function returns a one-element list
// the element is the expression statement, which
// we pass to evaluate
...
/*
example for string expression:
is_self_evaluating("'mystring';");
const my_string_expr = head(parse("'mystring';"));
evaluate(my_string_expr);

is_empty_list_expression(head(parse("[];")));
evaluate_empty_list_expression(head(parse("[];")));

evaluating example programs:
