...
function eval_fn_defn(expr, frame){
    ...
    display('fn_defn: '+expr);
    display('fname: '+fname);
    display('params: '+expr[1]);
    display('body: '+expr[2]);
    display('frame: '+frame);
    return insert(frame, fname, {params: expr[1], body: expr[2], frame: frame});
}
...
function apply_user_function(user_function, literal_call_params){
    ...
    const new_frame = head(make_child_frame(user_function.frame));
    display(array_length(literal_call_params));
    for(let i = 0; i < array_length(literal_call_params); i = i + 1){
        display(para);
        display(bd);
        display('cur_para: '+para[i+1]);
        insert(new_frame, para[i+1], literal_call_params[i]);
    }
    display('c');
    display('new_frame: '+new_frame);
...
display(evaluate_program([f, ["f", 10] ]));