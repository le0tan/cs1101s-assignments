...
function apply(fun, args) {
   if (is_builtin_function(fun)) {
      return apply_builtin_function(fun, args);
        ...
        return eval_comp(fun, head(args));
...
// make compound function objects
function make_comp(a, b){
    return {tag:"comp", f1: a, f2: b};
}

function is_comp(x){
    return is_tagged_object(x, "comp");
}

function eval_comp(x, val){
    const a = x.f1;
    const b = x.f2;
    // const env = x.env;
    return apply(a, list(apply(b, list(val))));
}

function is_unary_function(func){
    if(is_function_object(func)){
        return length(function_object_parameters(func)) === 1;
    } else {
        return false;
    }
}
...
parse_and_evaluate("function square(x) { return x * x;}\
