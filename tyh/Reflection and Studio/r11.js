...
function assign_name_to_value(name, value, env) {
    function env_loop(env) {
        if (is_empty_environment(env)) {
            ...
            if(!first_frame(env)[name].mutable){
                error("assignment to const is not allowed!");
            } else {
...
function _map(f, xs){
    ...
    if(is_empty_list(xs)){
        ...
        return pair(apply(f, list(head(xs))), _map(f, tail(xs)));
    }
}
...
parse_and_evaluate("map(x=>x+1, list(1,2,3));");